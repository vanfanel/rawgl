...
typedef struct {
  ...
  int                       loops;            // number of times the sample must be repeated. Has to be signed int because -1
                                              // means looping forever
  int                       loops_done;       // the number of times we have looped that sample
} sts_mixer_sample_t;
  ...
  for (; samples > 0; --samples) {
    ...
    for (i = 0; i < STS_MIXER_VOICES; ++i) {
      ...
      if (voice->state == STS_MIXER_VOICE_PLAYING) {
        ...
        } else {  // we have played the sample... but should we loop again?
            voice->sample->loops_done ++;
            if (voice->sample->loops_done < (voice->sample->loops + 1) || voice->sample->loops == (-1)) {
              voice->position = 0.0f;
            }
            else {
